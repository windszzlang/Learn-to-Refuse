...
from utils import extract_json_from_llama_output

current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
sys.path.append(parent_dir)

from llama_prompts.knowledge_a import KNOWLEDGE_A_PROMPT_TEMPLATE
from llama_prompts.knowledge_q import KNOWLEDGE_Q_PROMPT_TEMPLATE
from llama_prompts.main_qa import MAIN_QA_PROMPT_TEMPLATE
from llama_prompts.qa2knowledge import QA2KNOWLEDGE_PROMPT_TEMPLATE



# use cloud llama at deepinfra.com

class L2R_Llama_Chain:

    threshold = 0.75

    def __init__(self):
        self.openai_model = 'meta-llama/Llama-2-70b-chat-hf' # 'meta-llama/Llama-2-7b-chat-hf'
        self.temperature = 0
    ...
    def automatic_knowledge_enrich(self, number=20, batch_size=10, cache_dir='cache'):
        ...
        for i in range(int(number / batch_size) + 1):
            ...
            questions = extract_json_from_llama_output(questions)
            ...
            if j >= batch_size or i == len(new_question_list) - 1:
                ...
                if not os.path.exists(cache_path):
                    try:
                        ...
                        model_output = extract_json_from_llama_output(model_output)
    ...
    def answer(self, query: str, question=None, full_output=False, wikipedia=False, original_output=False) -> str:
        ...
        # print(result)
        result = extract_json_from_llama_output(result)
