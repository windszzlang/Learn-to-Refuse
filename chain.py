...
class L2R_Chain:
    ...
    def automatic_knowledge_enrich(self, number=20, batch_size=10):
        # step 1: get question
        question_list = []
        ...
        self.enrich_knowledge_from_truthfulqa(self, question_list, batch_size=10)
        return

    def enrich_knowledge_from_truthfulqa(self, question_list, batch_size=10):
        # step 1: get answer
        '''
        ...
        for i in tqdm(range(len(new_question_list))):
            ...
            if j >= batch_size or i == len(new_question_list) - 1:
                cache_path = 'cache/answer/' + str(i) +  '.json'
                ...
                j = 0
        # read cache
        for file in os.listdir('cache/answer/'):
            file_path = os.path.join('cache/answer/', file)
        ...
        for i in tqdm(range(len(qa_pairs))):
            ...
            if j >= batch_size or i == len(qa_pairs) - 1:
                cache_path = 'cache/knowledge/' + str(i) +  '.json'
                if not os.path.exists(cache_path):
                    try:
                ...
                j = 0
        # read cache
        for file in os.listdir('cache/knowledge/'):
            file_path = os.path.join('cache/knowledge/', file)
            if os.path.isfile(file_path):
                with open(file_path) as f:
